...
const BASE_CELL_CLASS =
  "relative h-16 overflow-hidden rounded text-center align-middle text-[10px] font-semibold sm:h-24 sm:text-sm";
...
export function GuessRow({ guess, isNew, columns, bg, indicator }: GuessRowProps) {
  ...
  return (
    <tr>
      <td
        className={`${BASE_CELL_CLASS} min-w-[104px] px-0 font-bold text-amber-50 sm:min-w-[180px]
        ${nameActive ? "bg-amber-950" : "bg-stone-700"}`}
      >
        {nameActive && (
          <div className="flex h-full w-full items-center gap-1.5 px-1.5 sm:gap-3 sm:px-3">
            <Image
...
export function PlaceholderGuessRow({ columns }: { columns: ColDef[] }) {
  return (
    <tr>
      <td className={`${BASE_CELL_CLASS} min-w-[104px] bg-stone-700 sm:min-w-[180px]`} />
      {columns.map((col) => (
        <td key={col.key} className={`${BASE_CELL_CLASS} w-[66px] bg-stone-700 sm:w-24`} />
      ))}
    </tr>
  );
}
