export type CellState = 'correct' | 'incorrect' | 'partial' | 'close' | 'higher' | 'lower';
...
export function compareGuess(guess: State, target: State): GuessResult {
  let coastline: CellState;
  if (guess.coastline === target.coastline) {
    coastline = 'correct';
  } else if (guess.coastline !== 'None' && target.coastline !== 'None') {
    coastline = 'partial';
  } else {
    coastline = 'incorrect';
  }
